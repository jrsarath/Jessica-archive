...
(function (root, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    // AMD + global
    define([], function () {
      return root.jessica = factory(root);
    });
  ...
  jessica = {
    ...
    /**
     * Initialize jessica with a list of commands to recognize.
     *
     * #### Examples:
     * ````javascript
     ...
     *
     * // initialize jessica, overwriting any previously added commands
     * jessica.init(commands, true);
     * // adds an additional command without removing the previous commands
     * jessica.init(commands2, false);
     * ````
     * As of v1.1.0 it is no longer required to call init(). Just start() listening whenever you want, and addCommands() whenever, and as often as you like.
     *
     * @param {Object} commands - Commands that jessica should listen to
    ...
    init: function init(commands) {
      ...
      // Sets the language to the default 'en-US'. This can be changed with jessica.setLanguage()
      ...
      recognition.onend = function () {
        ...
        // jessica will auto restart if it is closed automatically and not by user action.
        if (autoRestart) {
          // play nicely with the browser, and never restart jessica automatically more than once per second
          ...
          if (timeSinceLastStart < 1000) {
            setTimeout(function () {
              jessica.start({ paused: pauseListening });
      ...
      recognition.onresult = function (event) {
        if (pauseListening) {
          if (debugState) {
            logMessage('Speech heard, but jessica is paused');
          }
          return false;
        }
     ...
     *
     * - `autoRestart`  (boolean, default: true) Should jessica restart itself if it is closed indirectly, because of silence or window conflicts?
     * - `continuous`   (boolean) Allow forcing continuous mode on or off. jessica is pretty smart about this, so only set this if you know what you're doing.
     * - `paused`       (boolean, default: true) Start jessica in paused mode.
     *
     * #### Examples:
     * ````javascript
     * // Start listening, don't restart automatically
     * jessica.start({ autoRestart: false });
     * // Start listening, don't restart automatically, stop recognition after first phrase recognized
     * jessica.start({ autoRestart: false, continuous: false });
     * ````
     ...
     *
     * Alternatively, to only temporarily pause jessica responding to commands without stopping the SpeechRecognition engine or closing the mic, use pause() instead.
     * @see [pause()](#pause)
     *
     * @method abort
     */
    ...
    /**
     * Pause listening. jessica will stop responding to commands (until the resume or start methods are called), without turning off the browser's SpeechRecognition engine or the mic.
     *
     ...
     * @method setLanguage
     * @see [Languages](https://github.com/TalAter/jessica/blob/master/docs/FAQ.md#what-languages-are-supported)
     */
    ...
    /**
     * Add commands that jessica will respond to. Similar in syntax to init(), but doesn't remove existing commands.
     *
     * #### Examples:
     * ````javascript
     ...
     * jessica.addCommands(commands2);
     * // jessica will now listen to all three commands
     * ````
     *
     * @param {Object} commands - Commands that jessica should listen to
     * @method addCommands
     ...
     * jessica.removeCommands();
     *
     * // Add some commands
     * jessica.addCommands(commands);
     *
     * // Don't respond to hello
     * jessica.removeCommands('hello');
     *
     * // Don't respond to howdy or hi
     * jessica.removeCommands(['howdy', 'hi']);
     * ````
     ...
     *     This will fire once per Speech Recognition starting. See https://is.gd/jessica_sound_start
     ...
     * * `resultMatch` - Fired when jessica was able to match between what the user said and a registered command
     ...
     * #### Examples:
     * ````javascript
     * jessica.addCallback('error', function() {
     *   $('.myErrorText').text('There was an error!');
     * });
     *
     * jessica.addCallback('resultMatch', function(userSaid, commandText, phrases) {
     ...
     * jessica.addCallback('errorNetwork', notConnected, this);
     * ````
     ...
     * #### Examples:
     * ````javascript
     * jessica.addCallback('start', myFunction1);
     * jessica.addCallback('start', myFunction2);
     * jessica.addCallback('end', myFunction1);
     * jessica.addCallback('end', myFunction2);
     *
     * // Remove all callbacks from all events:
     * jessica.removeCallback();
     *
     * // Remove all callbacks attached to end event:
     * jessica.removeCallback('end');
     *
     * // Remove myFunction2 from being called on start:
     * jessica.removeCallback('start', myFunction2);
     *
     * // Remove myFunction1 from being called on all events:
     * jessica.removeCallback(undefined, myFunction1);
     * ````
     *
     ...
     * Returns false if speech recognition is off or jessica is paused.
     *
     * @return boolean true = SpeechRecognition is on and jessica is listening
     * @method isListening
     */
    ...
    /**
     * Returns the instance of the browser's SpeechRecognition object used by jessica.
     * Useful in case you want direct access to the browser's Speech Recognition engine.
     *
     * @returns SpeechRecognition The browser's Speech Recognizer currently used by jessica
     ...
     * #### Examples:
     * ````javascript
     * jessica.trigger('Time for some thrilling heroics');
     * jessica.trigger(
    ...
    trigger: function trigger(sentences) {
      if (!jessica.isListening()) {
        if (debugState) {
          if (!_isListening) {
            logMessage('Cannot trigger while jessica is aborted');
          } else {
            logMessage('Speech heard, but jessica is paused');
          }
        }
 ...
 *
 * jessica understands commands with `named variables`, `splats`, and `optional words`.
 *
 ...
 * <script>
 * var commands = {
 *   // jessica will capture anything after a splat (*) and pass it to the function.
 ...
 *
 *   // By defining a part of the following command as optional, jessica will respond
